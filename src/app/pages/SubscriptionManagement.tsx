...
export function SubscriptionManagement() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const [statusRes, historyRes] = await Promise.all([
        apiClient.checkSubscription(userId),
        apiClient.getSubscriptionHistory(userId)
      ]);
      setStatus(statusRes);
  ...
  return (
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      <div>
        <h1 className={`text-2xl font-bold ${textTitle}`}>Subscription Management</h1>
        <p className={textMuted}>Manage user premium plans and billing cycles.</p>
      </div>
        ...
        <form onSubmit={handleSearch} className="flex gap-4">
          <div className="flex-1 relative">
            ...
            <Input 
              ...
              placeholder="Enter User System UUID..." 
      ...
      {error && (
        <div className={`p-4 rounded-lg flex items-center gap-3 ${dm ? "bg-red-900/20 text-red-400 border border-red-800/40" : "bg-red-50 text-red-700 border border-red-100"}`}>
          <AlertCircle size={20} />
          <span>{error}</span>
        </div>
      )}
